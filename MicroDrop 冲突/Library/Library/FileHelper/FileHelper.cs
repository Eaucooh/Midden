...
namespace Library.FileHelper
{
    public class FileHelper
    {
        ...
        public static void Append(string path, string content) => WriteIn(path, $"{ReadAll(path)}{content}");
        ...
        public static Exp.Exception_Simple.Returning WriteByteIn(string path, byte[] content)
        {
            try
            {
                if (!File.Exists(path))
            ...
            {
                return new Exp.Exception_Simple.Returning() { SoF = false, Ex = p };
            }
        }
